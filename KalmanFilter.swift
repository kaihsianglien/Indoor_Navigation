...
class KalmanFilter : Filter {
    ...
    fileprivate var k: Double = 0.0 // Kalman gain
    fileprivate var p: Double = 0.0 // estimation error cvariance
    fileprivate var q: Double = 1.0 // process(predict) noise cvariance
    fileprivate var r: Double = 0.0 // measurement noise covariance
    fileprivate var x: Double = 0.0 // value
    
    func filter<T>(_ x: T, y: T, z: T) -> (T, T, T) {
        return (x, y, z)
    }
    ...
    func initFilter(_ deviceMotionUpdateInterval: Double) {
        ...
        F[1,4] = 1.0
        F[2,5] = 1.0
        F[0,6] = 1.0 // 1/2*deviceMotionUpdateInterval^2
        F[1,7] = 1.0 // 1/2*deviceMotionUpdateInterval^2
        F[2,8] = 1.0 // 1/2*deviceMotionUpdateInterval^2
        F[3,3] = 1.0
        F[4,4] = 1.0
        F[5,5] = 1.0
    ...
    func SetR(_ value: Double) {
        r = value
    }
}

func SimpleLinearRegression (_ x: [Double], y: [Double]) -> (Double, Double) {
...
func modulus(_ x: Double, y: Double, z: Double) -> Double {
    return sqrt((x ^ 2) + (y ^ 2) + (z ^ 2))
}

func modulusDifference(_ arr: [Double], avgModulus: Double) -> Double {
    var sum = 0.0
...
func roundNum(_ number: Double) -> Double {
